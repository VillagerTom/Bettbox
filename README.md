...
|---|---|---|
|Flutter|3.44.1||
|Go|1.25.x|1.20.x for old CPUs|
|Java|temurin-17.x||
|Android SDK|36.1||
|Android NDK|27.0.12077973<br>28.2.13676358||
...
2. 配置开发环境
  ...
  * Flutter
    ...
    * 从 git 仓库安装
      ...
      cd $installPath
      git clone -b 3.44.1 https:github.com/flutter/flutter.git
...
3. 配置构建环境
  * Visual C++ Build Tools
    ...
    下载 [Visual Studio 2022 (任意版本或生成工具) 引导程序](https://learn.microsoft.com/zh-cn/visualstudio/releases/2022/release-history#evergreen-bootstrappers) [^1] ，勾选“使用 C++ 的桌面开发”并安装

  * Inno Setup (ISCC.exe) [^2]
...
4. 构建并打包 [^3] [^4]
  ...
  dart setup.dart windows --compatible
  ```
...
4. 构建并打包 [^3] [^4]
  ...
  dart setup.dart linux --targets=deb,rpm,appimage --compatible
  ```
...
* Others
  ...
  ```bash
  dart setup.dart <platform> --out=core
  ```

连接并选择目标设备，按 F5 开始调试

[^1]: Flutter ≤3.38.2 与 Visual Studio 2026 不兼容，务必使用指定版本。更高版本 Flutter 请忽略

<!--TODO: 升级 flutter_distributor 插件？-->
[^2]: 当前版本 flutter_distributor (v0.4.2) 硬编码了 Inno Setup 的安装路径 (`C:\Program Files (x86)\Inno Setup 6`), 务必以管理员权限安装

[^3]: 使用 `--compatible` 参数为[旧 CPU](https://go.dev/wiki/MinimumRequirements#amd64) 构建

[^4]: 运行 `dart setup.dart help <platform>` 查看更多选项
